...
"""Unit tests for the P24 runner-local K-window decode planner.

These tests are pure-CPU: they exercise env parsing, the static/eligibility
gates, the schedule-output rewrite (budget clamp + KV allocation + fence
accounting) and the shortfall reconcile using duck-typed fakes, without any
NPU device or engine dependencies.
"""
...
def test_resolve_local_k_defaults_to_enabled(monkeypatch):
    monkeypatch.delenv(ENV_LOCAL_K, raising=False)
    assert resolve_local_k() == DEFAULT_LOCAL_K


def test_resolve_local_k_zero_disables():
    assert resolve_local_k("0") == 0
    assert resolve_local_k("-3") == 0


def test_resolve_local_k_clamps_to_max():
    assert resolve_local_k(str(MAX_LOCAL_K)) == MAX_LOCAL_K
    assert resolve_local_k(str(MAX_LOCAL_K + 10)) == MAX_LOCAL_K


def test_resolve_local_k_invalid_value_disables():
    assert resolve_local_k("not-a-number") == 0
    assert resolve_local_k("") == DEFAULT_LOCAL_K


def test_resolve_local_k_reads_env(monkeypatch):
    monkeypatch.setenv(ENV_LOCAL_K, "4")
    assert resolve_local_k() == 4
...
def test_scheduler_allows_window_positive():
    assert scheduler_allows_window(make_configured_scheduler()) is True


def test_scheduler_allows_window_scheduler_refusals():
    refusals = [
        {"num_spec_tokens": 2},
    ...
    for overrides in refusals:
        assert scheduler_allows_window(make_configured_scheduler(**overrides)) is False, overrides


def test_scheduler_allows_window_arch_refusal():
    scheduler = make_configured_scheduler()
    scheduler.vllm_config.model_config.architectures = ["SomeOtherModel"]
    assert scheduler_allows_window(scheduler) is False


def test_scheduler_allows_window_tp_refusal():
    scheduler = make_configured_scheduler()
    scheduler.vllm_config.parallel_config.tensor_parallel_size = 2
    assert scheduler_allows_window(scheduler) is False


def test_scheduler_allows_window_mamba_refusal():
    scheduler = make_configured_scheduler()
...
def test_plan_window_rewrites_single_decode_step():
    ...
    assert plan_lz_window(scheduler, output, 8) is True
    assert output.num_scheduled_tokens == {"r0": 8, "r1": 8}
    assert output.lz_window_steps == {"r0": 8, "r1": 8}
...
def test_plan_window_refused_when_new_reqs_scheduled():
    ...
    assert plan_lz_window(scheduler, output, 8) is False
    assert output.lz_window_steps == {}
...
def test_plan_window_refused_for_logprobs_or_penalties():
    ...
    scheduler = make_scheduler(requests)
    assert plan_lz_window(scheduler, make_scheduler_output(requests), 8) is False
...
def test_plan_window_budget_clamped_by_max_tokens():
    ...
    output = make_scheduler_output(requests)
    assert plan_lz_window(scheduler, output, 8) is True
    assert output.lz_window_steps["r0"] == 3
...
def test_plan_window_budget_clamped_by_model_len():
    ...
    output = make_scheduler_output(requests)
    assert plan_lz_window(scheduler, output, 8) is True
    assert output.lz_window_steps["r0"] == 4
...
def test_plan_window_preserves_confirmed_count_invariant():
    ...
    That identity is what keeps every subsequent schedule's ``num_new_tokens``
    exact: while a window is in flight the async scheduler optimistically
    counts the K in-flight samples as placeholders, and each received token
    consumes exactly one placeholder, so the engine-side confirmed length
    always matches the KV slots actually written.
    """
...
def test_plan_window_disabled_for_k_below_two():
    ...
    scheduler = make_scheduler(requests)
    output = make_scheduler_output(requests)
    assert plan_lz_window(scheduler, output, 0) is False
    assert plan_lz_window(scheduler, output, 1) is False
